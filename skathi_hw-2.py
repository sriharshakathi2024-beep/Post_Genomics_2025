# -*- coding: utf-8 -*-
"""
Created on Thu Sep 11 19:36:00 2025
...
def CaseIDs(vcffile):
    ...
    vcf_lines = vcffile.readlines()
    vcffile.close()

    for l in range(0, len(vcf_lines)): #looking line by line in the inputted VCF file
        if vcf_lines[l].startswith('##INDIVIDUAL'):

            ind1 = vcf_lines[l].find("ID=")
            ind2 = vcf_lines[l].find("NAME=")
...
def writecsv(inputfile, header):
    ...
    for record in vcf_reader:
        ...
        if len(tumorAD) == 3:
            depth_sum = tumorAD[0] + tumorAD[1] + tumorAD[2]
            x = tumorAD[0]
            y = tumorAD[1]
            z = tumorAD[2]

            # tri-allelic AD: x=REF, y=ALT1, z=ALT2 (already defined above)
            total = x + y + z
            if total > 0:
                fx, fy, fz = x/total, y/total, z/total
                # keep only if at least two of the three fractions are > 0.05
                if sum(v > 0.05 for v in (fx, fy, fz)) >= 2:
                    if y >= z:
                        refcount, altcount = x, y
                        chosen_alt = str(record.ALT[0])
                    else:
                        refcount, altcount = x, z
                        chosen_alt = str(record.ALT[1])

                    curr_var.append(record.CHROM)
                    left.append(record.POS)
                    right.append(record.POS + len(str(record.REF)))
                    ref_seq.append(record.REF)
                    alt_seq.append(chosen_alt)
                    ...
                    pass  # throw away variant
            else:
                pass  # zero depth; skip


        ########only 1 base for the ALT sequence
        else:
            depth_sum = tumorAD[0] + tumorAD[1]
            refcount = tumorAD[0]
            altcount = tumorAD[1]

            # both the REF and ALT have to be greater
            if (refcount / depth_sum) > 0.05 and (altcount / depth_sum) > 0.05:
                curr_var.append(record.CHROM)
                left.append(record.POS)
                right.append(record.POS + len(str(record.REF)))
                ref_seq.append(record.REF)
        ...
        info = []
        if len(normalAD) == 3:
            depth_sum = normalAD[0] + normalAD[1] + normalAD[2]
            x = normalAD[0]
            y = normalAD[1]
            z = normalAD[2]
            ...
            if total > 0:
                ...
                if sum(v > 0.05 for v in (fx, fy, fz)) >= 2:
                    ...
                    right1.append(record.POS + len(str(record.REF)))
                    ref_seq1.append(record.REF)
                    alt_seq1.append(chosen_alt)
        ...
        ########only 1 base for the ALT sequence
            ...
            depth_sum = normalAD[0] + normalAD[1]
            refcount = normalAD[0]
            altcount = normalAD[1]
    ...
    for i in range(normal.shape[0]):
        ...
        if len(normal['ref_seq'][i]) > len(normal['var_seq1'][i]):
    ...
    for j in range(tumor.shape[0]):
        if len(tumor['ref_seq'][j]) > len(tumor['var_seq1'][j]):
...
os.chdir(r"C:\Users\skathi\Downloads\hw2_vcf_files")
files = os.listdir()
#
for file in files:
